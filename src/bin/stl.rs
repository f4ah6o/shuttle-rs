...
fn resolve_remote_settings(
...
) -> Result<shuttle_rs::remote::RemoteSettings> {
    let saved = shuttle_rs::remote::RemoteSettings::load(env.shuttle_dir.join("remote.json")).ok();
    let url = overrides
        .url
        .clone()
    ...
    Ok(shuttle_rs::remote::RemoteSettings {
        url,
        project,
        token_env,
    })
...
fn remote_api(
    ...
    shuttle_rs::remote::HttpRemoteApi,
    shuttle_rs::remote::RemoteSettings,
)> {
    let settings = resolve_remote_settings(env, overrides)?;
