...
Worker)を、custom domain と Cloudflare Access を使ってデプロイする手順を
説明します。
...
- Workers と D1 が有効な Cloudflare アカウント。
- gateway hostname 用の zone を同じアカウントで管理していること。
- Node.js 20 以上と npm。
- ローカルにチェックアウトしたリポジトリ。
...
sample を `wrangler.toml` にコピーし、placeholder を置き換えます:
...
引き続きローカルの untracked な `wrangler.toml` で:

- `PUBLIC_URL` — デプロイされた Worker の base URL。この例では
  `https://shuttle.example.com` です。MCP/OAuth metadata に反映されます。
- `ADMIN_OWNER_ID` — 共有 tenant の固定 owner id。この例では
  `example-owner` です。
...
sample の設定は `workers_dev = false` と custom domain
`shuttle.example.com` を使います。`workers.dev` の公開 endpoint は有効に
しないでください。

最初の実クライアント接続前に、Cloudflare Access で
`shuttle.example.com/*` の Self-hosted application を作成し、次の allow policy
を設定します:

1. 人間の MCP client 用に専用 Access group を許可する policy。
...
team domain と application の AUD tag をローカルの `wrangler.toml` の
...
各 project では D1 の件数・内容・動作を確認して local `.shuttle` を削除した後、
...
- **deploy や migrate で `database_id` エラーになる** — ローカルの
  `wrangler.toml` が sample の placeholder id のままです。
  `wrangler d1 create shuttle`(または `wrangler d1 list`)の id を貼り付けてください。
