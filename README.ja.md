...
Shuttle は、現在の repository 向けに coding adapter（LoRA/PEFT adapter など）を
selection する軽量で local-first な project intelligence layer として動作します。
repository structure、git metadata、local event log から deterministic な project
embedding を構築し、local registry の adapter を cosine similarity で scoring し、
external inference engine 向けの routing manifest を export します。routing 自体は
model を実行しません。Shuttle は既に存在する adapter を scoring して export する
だけです。

```bash
# adapter を local registry に register する（embedding は adapter description から
# 計算されるか、--embedding '<json array>' で与えます）。
stl adapter register --name rust-cli \
  --base-model Qwen/Qwen2.5-Coder-7B-Instruct \
  --path /path/to/adapters/rust-cli --tag rust --tag cli
stl adapter list

# project embedding を構築し、selection を確認し、plan を生成する。
stl adapter index            # project embedding を構築して cache する
stl adapter select --json    # ranking された adapter と検出した project type
stl adapter merge --json     # 合計が 1.0 になる deterministic な weight
stl adapter export --json    # runtime manifest（base model + adapter path）
```

`merge` と `export` は `--top-k` と `--min-score` を受け取り、保持する adapter 数と
必要な最小 similarity を制御します。`export --format` は現状 `json` のみ対応で、
runtime 固有の preset（PEFT、vLLM、MLX）は予定です。

### doc-to-lora generation

既存 adapter への routing に加えて、Shuttle は external な
[doc-to-lora](https://github.com/SakanaAI/doc-to-lora) runner を駆動することで、
現在の project に蓄積された knowledge から adapter を生成できます。Shuttle は
repository metadata と local event log（decision、fact、pattern、observation、bug、
task、handoff、memory）から context document を組み立てて runner に渡し、生成された
adapter を register するので、すぐに routing 対象になります。model inference 自体は
Shuttle ではなく external runner が実行します。
...
stl adapter doc2lora \
  ...
  --tag generated \
  --focus "adapter routing"   # 任意: context document を bias して注釈する
```

Shuttle は context document を `<out-dir>/context.md` に書き出し、runner を次のように
起動します。
...
runner は `--output` 配下に adapter を生成し、JSON manifest を stdout に出力する必要が
あります。`path` は必須で、`base_model` と `name` は任意です。省略した場合は要求した値に
fallback します。
...
runner program は `--runner`、次に `SHUTTLE_DOC2LORA_RUNNER` 環境変数、最後に `PATH`
